...
- 默认最大代码行数：**50 行**
- 代码行数基于方法源码统计，默认忽略空行和仅包含 `{` / `}` 的行

### UI 组装/渲染/构建类函数

对于 UI 类方法，采用软/硬双阈值策略，允许更宽松的长度限制：

- **软阈值**：**80 行**代码（超过时以 warning 级别上报）
- **硬阈值**：**120 行**代码（超过时以 error 级别上报）
...
- `maxLines`：普通方法允许的最大代码行数（默认：50）
- `maxStmts`：兼容旧配置的别名参数，未设置 `maxLines` 时生效
- `maxUIStmtsSoft`：UI 类方法的软阈值，表示最大代码行数，超过时以 warning 上报（默认：80）
- `maxUIStmtsHard`：UI 类方法的硬阈值，表示最大代码行数，超过时以 error 上报（默认：120）
