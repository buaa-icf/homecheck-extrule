...
<head>
  ...
  <style>
    :root { color-scheme: dark; --bg:#08111f; --panel:#101d2f; --line:#22334b; --text:#e8f0fb; --muted:#91a3bb; --cyan:#22d3ee; --green:#34d399; --red:#fb7185; --amber:#fbbf24; }
    * { box-sizing: border-box; }
    body { margin:0; min-height:100vh; background:radial-gradient(circle at 15% 0,#123052 0,transparent 34%),var(--bg); color:var(--text); font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
    main { width:min(1500px,calc(100% - 32px)); margin:0 auto; padding:30px 0 48px; }
    header { display:flex; justify-content:space-between; align-items:flex-start; gap:20px; margin-bottom:22px; }
    h1 { margin:0 0 8px; font-size:clamp(24px,3vw,38px); letter-spacing:-.03em; background:linear-gradient(92deg,#e8f0fb 30%,#22d3ee); -webkit-background-clip:text; background-clip:text; color:transparent; }
    .subtitle,.muted { color:var(--muted); }
    .status { display:inline-flex; align-items:center; gap:8px; border:1px solid var(--line); background:#0d1929; border-radius:999px; padding:8px 12px; white-space:nowrap; }
    .dot { width:9px; height:9px; border-radius:50%; background:var(--amber); box-shadow:0 0 14px currentColor; }
    .status.completed .dot { background:var(--green); }.status.failed .dot { background:var(--red); }.status.running .dot { animation:pulse 1.4s infinite; }
    @keyframes pulse { 50% { opacity:.35; } }
    .cards { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:14px; margin-bottom:14px; }
    .card,.panel { background:color-mix(in srgb,var(--panel) 94%,transparent); border:1px solid var(--line); border-radius:16px; box-shadow:0 14px 40px rgba(0,0,0,.18); }
    .card { padding:16px 18px; min-height:96px; }
    .label { color:var(--muted); font-size:13px; margin-bottom:10px; }
    .value { font-size:24px; font-weight:700; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
    ...
    th { color:var(--muted); font-weight:600; background:#0d1929; }
    tbody tr:nth-child(even) { background:rgba(145,163,187,.05); }
...
<body>
  <main>
    ...
    <section class="panel table-panel"><h2 class="table-title">检测器性能</h2><div class="table-scroll"><table><thead><tr><th>仓库</th><th>规则</th><th>状态</th><th class="num">检测耗时 (s)</th><th class="num">告警数</th><th class="num">吞吐 (万行/s)</th><th>基准结果</th></tr></thead><tbody id="resultsBody"></tbody></table></div></section>
    <section class="panel table-panel"><h2 class="table-title">仓库共享资源</h2><div class="table-scroll"><table><thead><tr><th>仓库</th><th>规则</th><th>状态</th><th class="num">外层总耗时 (s)</th><th class="num">HomeCheck 分析 (s)</th><th class="num">峰值 RSS (MB)</th></tr></thead><tbody id="repositoriesBody"></tbody></table></div></section>
    <section class="panel table-panel" id="f1Panel"><h2 class="table-title">F1 评估（数据集仓库）</h2><div class="table-scroll"><table><thead><tr><th>规则</th><th class="num">TP</th><th class="num">FP</th><th class="num">FN</th><th class="num">Precision</th><th class="num">Recall</th><th class="num">F1</th></tr></thead><tbody id="f1Body"></tbody></table></div></section>
  </main>
  ...
  <script>
    ...
    (function () {
      ...
      function renderTable(){ var body=document.getElementById('resultsBody'); body.replaceChildren(); state.runs.forEach(function(run){ var row=document.createElement('tr'); var throughput=run.throughputWanLinesPerSecond; var target=throughput===null?'—':throughput>=throughputTarget?'已达到':'待提升'; var values=[run.repoName,run.smell,run.success?'成功':run.timedOut?'超时':'失败',(run.detectorDurationMs/1000).toFixed(2),String(run.issueMessages),throughput===null?'—':throughput.toFixed(4),target]; values.forEach(function(value,index){var cell=document.createElement('td');cell.textContent=value;if(index>=3&&index<=5)cell.className='num';if(index===2)cell.className=run.success?'ok':'bad';if(index===6&&throughput!==null)cell.className=throughput>=throughputTarget?'ok':'bad';row.appendChild(cell);});body.appendChild(row);}); }
      function renderRepositoryTable(){ var body=document.getElementById('repositoriesBody'); body.replaceChildren(); (state.repositoryRuns||[]).forEach(function(run){ var row=document.createElement('tr'); var values=[run.repoName,run.rules.join(', '),run.success?'成功':run.timedOut?'超时':'失败',(run.durationMs/1000).toFixed(2),(run.homecheckWallMs/1000).toFixed(2),run.peakRssMB.toFixed(2)]; values.forEach(function(value,index){var cell=document.createElement('td');cell.textContent=value;if(index>=3)cell.className='num';if(index===2)cell.className=run.success?'ok':'bad';row.appendChild(cell);});body.appendChild(row);}); }
